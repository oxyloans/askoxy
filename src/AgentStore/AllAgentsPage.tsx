      ...
      <main className="mx-auto max-w-7xl px-4 py-6">
        {loading && (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-purple-800 font-medium">Loading assistants, please wait…</p>
          </div>
        )}
        {error && (
          ...
          <>
            {data.assistants.length === 0 ? (
              <div className="text-center py-12 bg-white rounded-2xl shadow-sm border border-purple-100">
                ...
                <h3 className="text-xl font-semibold text-purple-800 mb-2">No assistants found</h3>
                <p className="text-purple-600 mb-6">Get started by creating your first AI assistant</p>
                <button
                  onClick={gotoStore}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs text-purple-700 mb-4">
                        <div><span className="font-medium">Domain:</span> {a.domain || "—"}</div>
                        <div><span className="font-medium">Subdomain:</span> {a.subDomain || "—"}</div>
                        <div><span className="font-medium">Target:</span> {a.targetUser || "—"}</div>
                        <div><span className="font-medium">Model:</span> {a.usageModel || "—"}</div>
                      </div>
                      ...
                      <div className="mb-4">
                        ...
                        {conv.length === 0 ? (
                          <div className="text-xs text-purple-400 italic">No conversation starters yet.</div>
                        ) : (
                          <ul className="space-y-2 max-h-32 overflow-y-auto">
                            {conv.slice(0, 2).map((c) => (
                              <li key={c.id} className="text-xs rounded-lg bg-purple-50 border border-purple-100 px-3 py-2">
                                <div className="font-medium text-purple-800">{c.conStarter1 || "—"}</div>
                                {c.conStarter2 && <div className="text-purple-600 mt-1">{c.conStarter2}</div>}
                            ...
                            {conv.length > 2 && (
                              <li className="text-xs text-center text-purple-500 py-1">+{conv.length - 2} more</li>
                            )}
                          </ul>
                        )}
                      </div>
                        ...
                        <label className="inline-flex items-center gap-2 text-xs cursor-pointer bg-purple-50 text-purple-700 hover:bg-purple-100 px-3 py-2 rounded-lg transition-colors">
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              else if (!a.assistantId) message.success("Missing assistantId for image upload.");
                              e.currentTarget.value = "";
                            }}
                          />
                        </label>
                      ...
                      {deleteConfirmId === a.id && (
                        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center px-4">
                          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
                            <h3 className="text-lg font-semibold text-purple-900 mb-2">Confirm Deletion</h3>
                            ...
                            <div className="flex justify-end gap-3">
                              ...
                              <button
                                onClick={() => deleteAssistant(a.assistantId, a.id)}
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center px-4 py-6">
          <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white shadow-lg p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-purple-900">Update Agent</h3>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <TextField label="Agent Name" value={editMap[showEdit]?.agentName || ""} onChange={(v) => onChangeDraft(showEdit, { agentName: v })} />
              <TextField label="Status" value={editMap[showEdit]?.status || ""} onChange={(v) => onChangeDraft(showEdit, { status: v })} />
              <TextField label="Agent Status" value={editMap[showEdit]?.agentStatus || ""} onChange={(v) => onChangeDraft(showEdit, { agentStatus: v })} />
              <TextField label="Domain" value={editMap[showEdit]?.domain || ""} onChange={(v) => onChangeDraft(showEdit, { domain: v })} />
              <TextField label="Sub Domain" value={editMap[showEdit]?.subDomain || ""} onChange={(v) => onChangeDraft(showEdit, { subDomain: v })} />
              <TextField label="Target User" value={editMap[showEdit]?.targetUser || ""} onChange={(v) => onChangeDraft(showEdit, { targetUser: v })} />
              <TextField label="Usage Model" value={editMap[showEdit]?.usageModel || ""} onChange={(v) => onChangeDraft(showEdit, { usageModel: v })} />
              <TextField
              ...
              />
              <TextField label="Tone" value={editMap[showEdit]?.converstionTone || ""} onChange={(v) => onChangeDraft(showEdit, { converstionTone: v })} />
              <ToggleField
                label="Active"
                value={!!editMap[showEdit]?.activeStatus}
                onChange={(v) => onChangeDraft(showEdit, { activeStatus: v as any })}
              />
              <ToggleField
                label="Voice"
                value={!!editMap[showEdit]?.voiceStatus}
                onChange={(v) => onChangeDraft(showEdit, { voiceStatus: v as any })}
              />
