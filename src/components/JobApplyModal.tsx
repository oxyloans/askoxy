...
const JobApplicationModal: React.FC<Props> = ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!sessionResumeUrl) {
        message.error(
          "No resume found in session. Please upload your resume first.",
        );
        return;
      }
      ...
      // 🔹 Read atsScoreViewerId stored by ExamResultsPage after exam submission
      const atsScoreViewerId = sessionStorage.getItem("atsScoreViewerId");
      ...
      // 🔹 Include atsScoreViewerId only if present (candidate came through exam flow)
      if (atsScoreViewerId) {
        payload.atsScoreViewerId = atsScoreViewerId;
      }
      ...
      // 🔹 Clean up all exam-related sessionStorage keys after successful apply
      sessionStorage.removeItem("resumeUrl");
      sessionStorage.removeItem("atsScoreViewerId");
          ...
          rules={[
            {
              validator: (_, value) => {
                ...
                if (value.trim().length > 1000)
                  return Promise.reject(
                    new Error("Cover letter must not exceed 1000 characters"),
                  );
