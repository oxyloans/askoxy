...
export default function SidebarLayout({ onLogout }: { onLogout: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-gray-900">
      ...
      {/* ══════════════════════════════════════════
          MOBILE TOPBAR — white, only on mobile
      ══════════════════════════════════════════ */}
      <header
        className="sticky top-0 z-[40] flex h-14 items-center justify-between border-b-2 border-slate-200 bg-white px-4 shadow-sm lg:hidden"
      ...
      >
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-xl border-2 border-slate-300 bg-slate-100 text-gray-700"
          >
            {mobileOpen ? <X size={18} /> : <Menu size={18} />}
          </button>

          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-blue-600 to-cyan-500 text-white shadow-md shadow-blue-200">
            <LayoutDashboard size={17} />
          </div>

          <div>
            <h1 className="text-[16px] font-black leading-tight text-gray-900">
              radhAI Admin
            </h1>
            <p className="text-[12px] font-bold leading-tight text-slate-600">
              CEO Control
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={logout}
            aria-label="Sign Out"
            className="flex h-10 w-10 items-center justify-center rounded-xl border-2 border-red-300 bg-red-50 text-red-600 transition active:scale-95 hover:bg-red-100"
            style={{ WebkitTapHighlightColor: "transparent" }}
          >
            <LogOut size={18} />
          </button>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-[41] lg:hidden">
          ...
          />
          <nav
            className="absolute bottom-0 left-0 top-14 flex w-[280px] flex-col border-r border-slate-700/50 bg-[#0f172a] shadow-2xl"
          ...
          >
              ...
              {SIDEBAR_ITEMS.map((item) => {
                ...
                return (
                  <NavLink
                    ...
                    onClick={() => setMobileOpen(false)}
                    className={({ isActive }) =>
                      `flex items-center gap-3 rounded-xl border px-3 py-3.5 transition-all active:scale-[0.98] ${
                        isActive
                          ? "shadow-sm"
                          : "border-transparent hover:border-slate-700 hover:bg-slate-800/50"
                      }`
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        ...
                        <div className="min-w-0 flex-1 overflow-hidden">
                          <span
                            className="block truncate text-[15.5px] font-semibold leading-tight"
                            style={{
                              color: isActive ? item.activeBg : "#cbd5e1",
                            }}
                          >
                          ...
                          <span
                            className="mt-0.5 block truncate text-[13px] font-normal leading-tight"
                            style={{
                              color: isActive ? item.accent : "#475569",
                            }}
                          >
            ...
            <div className="shrink-0 space-y-2 border-t border-slate-700/50 p-3">
              <div className="flex items-center gap-2.5 rounded-xl border border-slate-700/50 bg-slate-800/40 px-3 py-2.5">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-indigo-900/60">
                  <User size={15} className="text-indigo-300" />
                </div>
                <div className="min-w-0 flex-1 overflow-hidden">
                  <p className="truncate text-[14px] font-black text-slate-200">
                    radhAI.clone
                  </p>
              ...
              <button
                onClick={logout}
                className="flex w-full items-center justify-center gap-2 rounded-xl border border-red-900/40 bg-red-900/20 px-3 py-3 text-[14px] font-black text-red-400 transition hover:border-red-500/50 hover:bg-red-900/30 active:scale-95"
                style={{ WebkitTapHighlightColor: "transparent" }}
              >
                <LogOut size={16} />
                Sign Out
