...
const ExamPage: React.FC = () => {
  ...
  // 🔹 NEW: track loading state while we call /exam-status/start
  ...
  // 🔹 NEW: hits /exam-status/start before navigating to the question page
  const handleStart = async () => {
    if (!examData) return;

    if (!atsScoreHistoryId) {
      message.error("Missing exam session. Please try again.");
      return;
    }
    ...
    try {
      ...
      if (startRes.data?.status !== true) {
        message.error(startRes.data?.message || "Failed to start exam");
        setStartingExam(false);
        return;
      }

      // 🔹 the new record returned by startExam — use its id going forward
      const newAtsScoreHistoryId = startRes.data?.data?.id;

      if (!newAtsScoreHistoryId) {
        message.error(
          "Exam session could not be initialized. Please try again.",
        );
        setStartingExam(false);
        return;
      }
      ...
      navigate(`/main/exam/question/1`, {
        state: {
          ...
          jobId,
          fileUrl,
          atsScoreHistoryId: newAtsScoreHistoryId, // 🔹 updated id passed forward
        },
      });
    } catch (err) {
      console.error(err);
      message.error("Something went wrong while starting the exam");
    } finally {
      setStartingExam(false);
    }
  };
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-4 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-lg w-full">
          <div className="relative w-44 h-44 mx-auto mb-10">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                ...
                >
                  {Math.round(pollProgress)}%
                </div>
                <div className="text-indigo-600 text-[10px] uppercase tracking-[3px] mt-1 font-bold">
                  Ready
                </div>
              </div>
            </div>
          ...
          <p className="text-slate-600 text-base mb-8">
            AI is generating your custom assessment. This takes 30–60 seconds.
          </p>
          <div className="bg-white border border-slate-200 rounded-2xl p-5 text-left space-y-3 shadow-sm">
            {[
              ...
              "Finalizing difficulty levels",
            ].map((step, i) => (
              <div key={i} className="flex items-center gap-3">
                <div
                  className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 transition-all ${pollProgress > (i + 1) * 22 ? "bg-indigo-500" : "bg-slate-100"}`}
                >
                  {pollProgress > (i + 1) * 22 ? (
                ...
                <span
                  className={`text-sm font-medium transition-colors ${pollProgress > (i + 1) * 22 ? "text-slate-900 font-bold" : "text-slate-400"}`}
                >
                  {step}
                </span>
              </div>
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-slate-600 mb-8">
            We couldn't generate your exam. Please try again.
          </p>
          <Button
            size="large"
            onClick={() => navigate(-1)}
            className="!bg-indigo-600 !border-0 !text-white !rounded-2xl !font-bold !h-12 !px-8"
          >
            Go Back
          </Button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-white pb-20">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-10 space-y-8 d">
        {/* Hero */}
        <div
          className="relative rounded-[32px] overflow-hidden p-8 sm:p-12"
          style={{
            background: "linear-gradient(135deg,#f0f4ff,#e0e7ff 60%,#f0f4ff)",
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-8">
              <div
                className="w-20 h-20 rounded-3xl flex-shrink-0 flex items-center justify-center shadow-[0_12px_40px_rgba(251,191,36,0.25)]"
                style={{
              ...
              <div>
                <div className="flex flex-wrap gap-2 mb-3">
                  <span className="px-3 py-1 rounded-lg text-xs font-black bg-amber-100 text-amber-800">
                    🎉 CONGRATULATIONS
                  </span>
                ...
                <h1 className="s text-2xl sm:text-3xl font-black text-slate-900 mb-1">
                  You're Selected for the Exam!
                </h1>
                <p className="text-indigo-900 font-semibold text-base">
                  {jobDesignation} · {companyName}
                </p>
              </div>
            </div>
            <p className="text-indigo-950 text-base leading-relaxed max-w-2xl mb-8">
              Your profile cleared the 80% threshold. Complete this AI-generated
              skills assessment — one question at a time, 60 seconds each — to
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                },
                {
                  ...
                  label: "Difficulty",
                  value: examData?.difficultyLevel ?? "—",
                },
                {
                  icon: <BulbOutlined />,
                  label: "Skills",
                  value: examData?.summary?.skillsCovered?.length ?? "—",
                },
              ].map((s2) => (
                <div
                  key={s2.label}
                  className="bg-white/70 border border-indigo-200 rounded-2xl p-4 text-center shadow-sm"
                >
                  <div className="text-indigo-600 text-lg mb-1">{s2.icon}</div>
                  <div className="s text-slate-900 font-black text-xl">
                    {s2.value}
                  </div>
                  <div className="text-indigo-500 text-xs uppercase tracking-wider mt-0.5 font-bold">
                    {s2.label}
                  </div>
                </div>
              ))}
        ...
        {examData && (
          <div className="bg-slate-50 border border-slate-200 rounded-[24px] p-6 sm:p-8">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h3 className="s text-slate-900 font-black text-lg leading-none">
                  Instructions
                </h3>
            ...
            <div className="pt-5 border-t border-slate-200">
              <p className="text-slate-600 text-xs uppercase tracking-widest font-bold mb-3">
                Skills Assessed
              </p>
              <div className="flex flex-wrap gap-2">
                {examData.summary?.skillsCovered?.map(
                  (sk: string, i: number) => (
                    <span
                      key={i}
                      className="px-3 py-1 rounded-lg text-xs font-bold bg-[#f0f3ff] text-[#4f46e5] border border-[#c7d2fe]"
                    >
                      {sk}
                    </span>
                  ),
        ...
        <div className="text-center py-8">
          <button
            onClick={handleStart}
            disabled={examStatus !== "ready" || startingExam}
            style={{
              height: 64,
              padding: "0 72px",
              borderRadius: 20,
              ...
              background:
                examStatus === "ready" && !startingExam
                  ...
                  : "#f1f5f9",
              color:
                examStatus === "ready" && !startingExam ? "#fff" : "#94a3b8",
              boxShadow:
                examStatus === "ready" && !startingExam
          ...
          <p className="text-slate-500 text-sm mt-4">
            {startingExam
              ? "Initializing your exam session..."
              : examStatus === "ready"
                ? "1 question at a time · 60 sec each · No going back"
                : "Waiting for exam to finish generating..."}
          </p>
        </div>
      </div>
    </div>
