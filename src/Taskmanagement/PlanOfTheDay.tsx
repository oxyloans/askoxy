...
import React, { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom"; // UPDATE: Added for redirection
import UserPanelLayout from "./UserPanelLayout";
import BASE_URL from "../Config";
import {
  Form,
...
import {
  ...
  EditOutlined,
  StarOutlined,
  ExclamationCircleOutlined, // UPDATE: Added for warning icon
} from "@ant-design/icons";
import axios from "axios";
...
interface Task {
  ...
  planCreatedAt: string;
  lastFourDigitsUserId: string;
  endOftheDay?: string | null; // UPDATE: Added to track EOD status
}

const PlanOfTheDay: React.FC = () => {
  const [form] = Form.useForm<TaskFormValues>();
  const navigate = useNavigate(); // UPDATE: Added for redirection
  const [loading, setLoading] = useState(false);
  const [fetchingStatus, setFetchingStatus] = useState(true);
  const [userId, setUserId] = useState("");
  const [userName, setUserName] = useState("");
  ...
  const [errorState, setErrorState] = useState<boolean>(false);
  const [submissionSuccess, setSubmissionSuccess] = useState<boolean>(false);
  const [pendingEodTasks, setPendingEodTasks] = useState<Task[]>([]); // UPDATE: Added to track pending EOD tasks
  ...
  useEffect(() => {
    ...
    // UPDATE: Check both today's task and pending EOD tasks
    checkPendingTasks(storedUserId);
  }, []);

  // UPDATE: Modified to check both today's task and pending EOD tasks
  const checkPendingTasks = async (userIdValue: string) => {
    setFetchingStatus(true);
    setErrorState(false);
    try {
      ...
      const foundTodayTask = tasks.find((task) => {
        const taskDateStr = task.planCreatedAt.split(" ")[0];
        return taskDateStr === todayStr && task.taskStatus === "PENDING";
      });

      // UPDATE: Find pending EOD tasks (tasks with no endOftheDay from previous days)
      ...
      setPendingEodTasks(pendingTasks);
    } catch (error: any) {
  ...
  const onFinish = async (values: TaskFormValues) => {
    // UPDATE: Prevent submission if there are pending EOD tasks
    if (pendingEodTasks.length > 0) {
      notification.warning({
        message: "Pending Tasks",
        description:
          "Please complete the End of Day updates for previous tasks before submitting a new plan.",
        placement: "topRight",
      });
      return;
    }
  ...
  // UPDATE: Added handler for redirecting to TaskUpdate
  const handleCompletePendingEod = () => {
    sessionStorage.setItem("fromPlanOfTheDay", "true"); // Flag for redirection
    navigate("/taskupdated");
  };
  ...
  const renderSubmittedState = () => (
    <div className="flex flex-col items-center justify-center py-4">
      ...
      {todayTask && (
        <Card className="w-full mt-2 bg-white border border-gray-200 shadow-sm rounded-xl text-left hover:shadow-md transition-all">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-2 sm:mb-4">
            <div className="flex items-center gap-2">
              <Text strong className="text-base sm:text-lg text-gray-800">
                Today Plan Of The Day
  ...
  // UPDATE: Added warning state for pending EOD tasks
  const renderPendingEodWarning = () => (
    <Result
      status="warning"
      title="Pending End of Day Updates"
      subTitle={
        <span>
          You have {pendingEodTasks.length} pending End of Day update(s) from
          previous days. Please complete them before submitting a new plan.
        </span>
      }
      extra={
        <Button
          type="primary"
          onClick={handleCompletePendingEod}
    ...
    >
      ...
      <Form.Item className="mb-0">
        <Button
          ...
          disabled={pendingEodTasks.length > 0} // UPDATE: Disable button if pending EOD tasks exist
        >
          {submissionSuccess
            ? "Plan Submitted!"
            : loading
        ...
        >
          ...
          <div className="p-5 md:p-6 lg:p-8">
            {fetchingStatus
              ...
              : errorState
              ? renderErrorState()
              : pendingEodTasks.length > 0 // UPDATE: Check for pending EOD tasks
              ? renderPendingEodWarning()
              : isSubmitted
              ? renderSubmittedState()
              : renderFormState()}
          </div>
