...
const TripResult: React.FC<TripResultProps> = ({ tripResult, formData }) => {
      ...
      if (response.ok) {
        ...
        audioContainer.innerHTML = `
          <div style="background: #f0fdf4; padding: 0.5rem; border-radius: 8px; margin-top: 0.5rem; border: 1px solid #bbf7d0;">
            <h4 style="color: #059669; margin-bottom: 0.5rem;">🎧 Audio Summary Generated</h4>
  ...
  const handleShareTrip = () => {
    ...
    const shareText = `🌍 ${formData.city} Trip Plan\n📅 ${formData.num_days} days, ${formData.no_of_persons} travelers\n💰 Budget: ${formData.budget_range}\n${formData.multi_cities.length > 0 ? `🗺️ Multi-city: ${formData.multi_cities.join(', ')}\n` : ''}\n✨ Generated by AskOxy AI Trip Planner\n🌐 askoxy.ai`;
  ...
  if (!tripResult || !formData) {
    return (
      <div className="result-container">
        <div className="result-title">Your Perfect Trip Awaits ✈️</div>
  ...
  const currencySymbols: { [key: string]: string } = {
    'USD': '$', 'EUR': '€', 'GBP': '£', 'JPY': '¥', 'INR': '₹', 'CAD': 'C$', 'AUD': 'A$', 'CNY': '¥', 'SGD': 'S$', 'AED': 'د.إ'
  };
          ...
          <div className="pricing-content">
            ...
            <div className="cost-summary">
              <div className="cost-item">
                <span className="cost-label">✈️ Flights</span>
              ...
              {pricing?.multi_city > 0 && (
                <div className="cost-item">
                  <span className="cost-label">🗺️ Multi-City</span>
            ...
            {(tripResult as any).pricing_data?.group_discount > 0 && (
              <div className="company-offer">
                <h4>🎉 Group Discount Applied: {(tripResult as any).pricing_data.group_discount}% OFF</h4>
                <p>You're saving money by traveling as a group!</p>
              </div>
            )}
            <div className="company-offer">
              <h4>🎆 AskOxy Travel Packages</h4>
      ...
      case 'packages':
        return (
          <div className="packages-content">
            <h3>🎆 AskOxy Travel - Pre-Planned Packages</h3>
            ...
            <div className="packages-grid">
              <div className="package-card budget">
                ...
                <div className="package-duration">{formData.num_days} Days • {formData.no_of_persons} Travelers</div>
                <ul className="package-features">
                  <li>✈️ Economy flights included</li>
                  <li>🏨 3-star hotel accommodation</li>
                  <li>🍽️ Daily breakfast</li>
                  <li>🗺️ City tour guide</li>
                  <li>📞 24/7 support</li>
                </ul>
            ...
            <div className="company-contact">
              <h4>📞 Ready to Book? Contact AskOxy Travel</h4>
              <div className="contact-info">
                <span>📧 travel@askoxy.ai</span>
                <span>📞 +1 (555) ASKOXY</span>
                <span>🌐 www.askoxy.ai/travel</span>
              </div>
            </div>
          </div>
        );
          ...
          <div className="weather-content">
            ...
            {weather?.travel_month && (
              <p style={{textAlign: 'center', marginBottom: '1rem', color: '#6b7280'}}>
                📅 Travel Month: {weather.travel_month}
              </p>
            )}
            <div className="weather-card">
              <div className="weather-main">
                <span className="temp">{weather?.current?.temperature || 22}°C</span>
                <span className="condition">{weather?.current?.condition || 'Partly Cloudy'}</span>
              </div>
              <div className="weather-details">
                <span>💧 Humidity: {weather?.current?.humidity || 65}%</span>
                <span>🌬️ Wind: {weather?.current?.wind_speed || 12} km/h</span>
              </div>
            </div>
            {weather?.forecast && (
                ...
                <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: '0.5rem'}}>
                  {weather.forecast.slice(0, 5).map((day: any, index: number) => (
                    <div key={index} style={{background: '#f8fafc', padding: '0.5rem', borderRadius: '6px', textAlign: 'center'}}>
                      ...
                      <div style={{fontSize: '0.75rem', color: '#6b7280'}}>{day.high}°/{day.low}°</div>
                    </div>
                  ))}
                </div>
              </div>
            )}
            <p className="weather-tip">🌟 {weather?.best_time || 'Spring and Fall offer pleasant weather'}</p>
          </div>
        );
          ...
          <div className="safety-content">
            ...
            {safety?.current_alerts && safety.current_alerts.length > 0 && (
              <div style={{background: '#fef3c7', padding: '1rem', borderRadius: '8px', margin: '1rem 0', border: '1px solid #f59e0b'}}>
                <h4 style={{color: '#d97706', margin: '0 0 0.5rem 0'}}>⚠️ Current Alerts</h4>
            ...
            <div className="safety-tips">
              ...
              {safety?.emergency_numbers && (
                <div style={{background: '#f0fdf4', padding: '1rem', borderRadius: '8px', marginTop: '1rem'}}>
                  <h4 style={{color: '#059669', margin: '0 0 0.5rem 0'}}>🆘 Emergency Numbers</h4>
                  <div style={{display: 'flex', gap: '1rem', flexWrap: 'wrap'}}>
                    <span>🚑 Police: {safety.emergency_numbers.police}</span>
                    <span>🏥 Medical: {safety.emergency_numbers.medical}</span>
                    <span>🚒 Fire: {safety.emergency_numbers.fire}</span>
                  </div>
                </div>
              )}
            </div>
    ...
    <div className="result-container">
      <div className="result-content">
        <div className="result-header">
          <h2>🌍 {formData.budget_range.charAt(0).toUpperCase() + formData.budget_range.slice(1)} Trip to {formData.city}</h2>
          <div className="trip-summary">
            <span className="summary-item">📅 {formData.num_days} Days</span>
            <span className="summary-item">👥 {formData.no_of_persons} Travelers</span>
            <span className="summary-item">💰 {formData.currency}</span>
            {formData.multi_cities.length > 0 && <span className="summary-item">🗺️ Multi-City</span>}
          </div>
        </div>
