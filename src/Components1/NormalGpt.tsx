...
const NormalGpt: React.FC = () => {
  ...
  // Dummy rice topics
  const riceTopics = [
    { id: 1, title: "Ask questions", content: "Discover useful tips!" },
    { id: 2, title: "Ask mentorship", content: "Seek expert guidance!" },
    { id: 3, title: "Ask for funds", content: "Learn funding strategies." },
    {
      id: 4,
      ...
      showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded-lg p-8 shadow-xl max-w-sm sm:max-w-md w-full">
            {/* Modal Message */}
            <p className="text-sm text-gray-700 mb-6 text-center font-medium">
              You are currently browsing as a guest. To unlock full access and
              enjoy all features, please sign in to your account.
            </p>
            {/* Action Buttons */}
            <div className="flex justify-center items-center space-x-6">
              <button
                onClick={() => setShowModal(false)}
                className="px-6 py-3 text-sm text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 focus:outline-none transition-all duration-300"
              >
                Continue as Guest
              </button>
              <button
                onClick={handleSignIn}
                className="px-6 py-3 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:outline-none transition-all duration-300 transform hover:scale-105"
              >
                Sign In Now
              </button>
            </div>
