...
const OrdersPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!loading && orders.length > 0) {
      const savedOrderId = sessionStorage.getItem("partner_orderId");
      if (savedOrderId) {
  ...
  const handleOrderDetails = (order: Order | ExchangeOrder) => {
    ...
    sessionStorage.setItem("partner_orderId", orderId);
    if (tableRef.current) {
      sessionStorage.setItem("partner_scrollPosition", window.scrollY.toString());
    }
