...
const FreelancerProfiles: React.FC = () => {
  ...
  // filter((freelancer) => Number(freelancer.status) !== 0)
  const fetchFreelancers = async () => {
    try {
      setLoading(true);
      setPageError(null);
      const res = await freelanceApi.get<Freelancer[]>(
        `${BASE_URL}/ai-service/agent/getAllFreeLancers`,
      );
     setFreelancers(
       Array.isArray(res.data)
         ? res.data.filter((freelancer) => freelancer.status !== 0)
         : [],
     );
    } catch (err: unknown) {
  ...
  const handleAssign = async (f: Freelancer) => {
    ...
    try {
      ...
      const res = await freelanceApi.post(
        `${BASE_URL}/user-service/partnerAssignedFreelancer`,
        {
          companyId,
          id: requirementId,
          freelancerId: f.userId1,
        },
      );
      if (res.status === 200 || res.status === 201) {
        const msg =
          extractResponseMessage(res.data) ||
        ...
        setAssignFeedback({
          text: extractResponseMessage(res.data) || "Could not assign.",
          variant: "error",
        });
      }
    } catch (err: unknown) {
      setAssignFeedback({ text: extractApiError(err), variant: "error" });
    } finally {
      setAssigning(false);
      setAssigningId(null);
    }
  ...
  if (loading)
    return (
      <EmployeeLayout>
        <LoadingCenter tip="Loading freelancers…" />
      </EmployeeLayout>
    );
  ...
  return (
    <EmployeeLayout>
      ...
      <div className={pageContainerClass}>
        ...
        <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold text-gray-900 sm:text-2xl">
              Browse Freelance Talent
            </h1>
            <p className="mt-0.5 text-sm text-gray-500">
              Discover skilled freelancers and assign them to your open
              requirements
            </p>
          </div>
        ...
        {assignFeedback && (
          <StatusAlert
            message={assignFeedback.text}
            variant={assignFeedback.variant}
            onDismiss={() => setAssignFeedback(null)}
            className="mb-5"
          />
        )}
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-xl border border-gray-100 bg-white py-20 text-center shadow-sm">
            ...
            <p className="mt-1 text-sm text-gray-400">
              Try a different rate filter or check back later
            </p>
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((f) => (
              <div
                key={f.userId1}
                className="flex flex-col rounded-xl border border-gray-100 bg-white shadow-sm transition hover:shadow-md hover:-translate-y-0.5"
              >
                {/* Card top */}
                ...
                <div className="mx-5 mb-4 flex items-center justify-between rounded-lg border border-gray-100 bg-gray-50 px-4 py-2.5">
                  ...
                  <span
                    className={`text-xs font-bold ${f.amountNegotiable === "YES" ? "text-emerald-600" : "text-red-500"}`}
                  >
                ...
                <div className="mt-auto flex gap-2 border-t border-gray-50 px-5 py-4">
                  ...
                  {companyId && requirementId && (
                    ...
                    >
                      {assigning && assigningId === f.userId1
                        ? "Assigning…"
                        : "Assign to Role"}
                    </button>
                  )}
