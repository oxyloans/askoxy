...
const ActiveMembership: React.FC = () => {
  const [records, setRecords] = useState<ActiveMembershipItem[]>([]);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `${BASE_URL}/ai-service/entity-records/active-membership`,
        {
          params: { page, size },
        },
      );
  ...
  const columns: ColumnsType<ActiveMembershipItem> = [
    {
    ...
    },
    {
      ...
      key: "name",
      align: "center",
      render: (_value, record: ActiveMembershipItem) => {
      ...
      >
        <div>
          ...
          <Text type="secondary" style={{ fontSize: 13 }}>
            View Active Membership contact records
          </Text>
        </div>
        <Button
          icon={<ReloadOutlined />}
