...
interface JourneySummaryRow {
  ...
  campaignsWithoutInterest: number;
  interestedMobiles: string[];
  campaignTypes: string[];
  latestInterestedAt: number;
}

type CampaignFilter = "ALL" | "ACTIVE" | "INACTIVE" | "NO_INTEREST";
...
const JourneysVsCampaignsVsInterestedUsers: React.FC = () => {
  ...
  const summaryRows = useMemo<JourneySummaryRow[]>(() => {
    return journeys.map((journey) => {
      ...
      const users = campaigns.flatMap((campaign) => campaign.interestedUsers || []);
      ...
      return {
        ...
        activeCampaignCount: campaigns.filter((campaign) => campaign.campaignStatus).length,
        interestedUsersCount: users.length,
        campaignsWithoutInterest: campaigns.filter(
          (campaign) => !(campaign.interestedUsers || []).length
        ).length,
        interestedMobiles,
        campaignTypes: campaigns.map((campaign) => campaign.campaignType).filter(Boolean),
  ...
  const filteredJourneys = useMemo(() => {
    ...
    return summaryRows.filter((row) => {
      return (
        ...
        row.interestedMobiles.some((mobile) => mobile.toLowerCase().includes(term))
      );
    });
  ...
  const totalActiveCampaigns = useMemo(
    () => summaryRows.reduce((sum, journey) => sum + journey.activeCampaignCount, 0),
    [summaryRows]
  );
  const totalInterestedUsers = useMemo(
    () => summaryRows.reduce((sum, journey) => sum + journey.interestedUsersCount, 0),
    [summaryRows]
  );
  const campaignsWithoutInterest = useMemo(
    () => summaryRows.reduce((sum, journey) => sum + journey.campaignsWithoutInterest, 0),
    [summaryRows]
  );

  const selectedJourney = useMemo(
    () => journeys.find((journey) => journey.journeyId === selectedJourneyId) || null,
    [journeys, selectedJourneyId]
  );

  const selectedJourneySummary = useMemo(
    () => summaryRows.find((journey) => journey.journeyId === selectedJourneyId) || null,
    [summaryRows, selectedJourneyId]
  );

  const visibleCampaigns = useMemo(() => {
    const campaigns = selectedJourney?.campaigns || [];

    if (campaignFilter === "ACTIVE") {
      return campaigns.filter((campaign) => campaign.campaignStatus);
    }
    if (campaignFilter === "INACTIVE") {
      return campaigns.filter((campaign) => !campaign.campaignStatus);
    }
    if (campaignFilter === "NO_INTEREST") {
      return campaigns.filter((campaign) => !(campaign.interestedUsers || []).length);
    }
    return campaigns;
  }, [selectedJourney, campaignFilter]);
  ...
  useEffect(() => {
    ...
    const currentExists = journeys.some(
  ...
  const metricCards = [
    ...
    },
    {
      ...
      value: campaignsWithoutInterest,
      note: "Campaigns with zero interest",
        ...
        <div className="mx-auto flex max-w-[1600px] flex-col gap-3 px-3 py-3 sm:px-5 md:flex-row md:items-center md:justify-between lg:px-7">
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-3xl text-xs leading-5 text-slate-500 sm:text-sm">
              Review journey coverage, campaign activity and user interest from one focused admin workspace.
            </p>
          </div>
      ...
      <main className="mx-auto max-w-[1600px] space-y-4 px-3 py-4 sm:px-5 lg:px-7 lg:py-6">
          ...
          <div className="flex flex-col gap-3 border-t border-slate-200 px-4 py-3 sm:px-5 md:flex-row md:items-center md:justify-between">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="text-xs text-slate-600">
                <span className="font-bold text-slate-900">{activePercent}%</span> campaign activity
              </div>
            </div>
            <div className="text-[11px] text-slate-400">
              Flow: Journey → Campaign → Interested User
            </div>
          </div>
        </section>
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-slate-500 sm:text-sm">
                  Select a journey, inspect its campaigns, then review the users who showed interest.
                </p>
              </div>
          ...
          {loading ? (
            ...
            <div className="p-10">
              <Empty description="No journey data matched your search" />
            </div>
          ) : (
            <>
              ...
              <div className="grid min-h-[650px] lg:grid-cols-[340px_minmax(0,1fr)] xl:grid-cols-[380px_minmax(0,1fr)]">
                <aside className="hidden border-r border-slate-200 bg-slate-50/70 lg:block">
                  <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
                    <div>
                      <div className="text-sm font-extrabold text-slate-900">All Journeys</div>
                      <div className="text-[11px] text-slate-500">
                        {filteredJourneys.length} of {summaryRows.length} shown
                      </div>
                    </div>
                    <Tag className="m-0">{totalJourneys}</Tag>
                  </div>

                  <div className="max-h-[760px] overflow-y-auto">
                    {filteredJourneys.map((journey) => {
                      const selected = journey.journeyId === selectedJourneyId;
                        ...
                        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                          <div className="min-w-0">
                            ...
                            <div className="mt-3 flex flex-wrap gap-2">
                              <Tag className="m-0">{selectedJourneySummary.campaignCount} campaigns</Tag>
                              <Tag color="green" className="m-0">
                                {selectedJourneySummary.activeCampaignCount} active
                              </Tag>
                              <Tag
                                color={selectedJourneySummary.interestedUsersCount ? "blue" : "gold"}
                                className="m-0"
                              >
                                {selectedJourneySummary.interestedUsersCount} interested users
                              </Tag>
                              {selectedJourneySummary.campaignsWithoutInterest > 0 && (
                                <Tag color="orange" className="m-0">
                                  {selectedJourneySummary.campaignsWithoutInterest} need attention
                                </Tag>
                              )}
                            </div>
                          </div>
                      ...
                      <div className="grid xl:grid-cols-[minmax(0,1.05fr)_minmax(340px,.95fr)]">
                        <div className="border-b border-slate-200 xl:border-b-0 xl:border-r">
                          <div className="flex items-center justify-between px-4 py-3 sm:px-5 lg:px-6">
                            <div>
                              <div className="text-sm font-extrabold text-slate-900">Campaigns</div>
                              <div className="text-[11px] text-slate-500">
                                Select a campaign to inspect user interest
                              </div>
                            </div>
                            <Tag className="m-0">{visibleCampaigns.length}</Tag>
                          </div>

                          {!visibleCampaigns.length ? (
                            <div className="border-t border-slate-100 p-8">
                              <Empty description="No campaigns match this filter" />
                            </div>
                          ) : (
                            <div className="max-h-[590px] divide-y divide-slate-100 overflow-y-auto border-t border-slate-100">
                              {visibleCampaigns.map((campaign, index) => {
                                ...
                                const latest = users.reduce(
                                  (max, user) => Math.max(max, toMs(user.createdAt)),
                                  0
                                );
                                    ...
                                    <div className="flex items-start gap-3">
                                      ...
                                      <div className="min-w-0 flex-1">
                                        <div className="break-words text-sm font-extrabold leading-5 text-slate-900">
                                          {campaign.campaignType || "Unnamed campaign"}
                                        </div>
                                        ...
                                        <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
                                          <Tag
                                            color={campaign.campaignStatus ? "success" : "default"}
                                            className="m-0"
                                          >
                                            {campaign.campaignStatus ? "ACTIVE" : "INACTIVE"}
                                          </Tag>
                        ...
                        <div className="bg-slate-50/70">
                            ...
                            <div className="flex items-center justify-between gap-3">
                              ...
                              {selectedCampaign && (
                                <Tag color="blue" className="m-0">
                                  {(selectedCampaign.interestedUsers || []).length}
                                </Tag>
                              )}
                            </div>
                          </div>
                          ...
                          {!selectedCampaign ? (
                          ...
                          ) : (selectedCampaign.interestedUsers || []).length === 0 ? (
                            <div className="p-5 sm:p-8">
                              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-center">
                                ...
                                <div className="mt-1 text-xs leading-5 text-amber-700">
                                  This campaign currently has no user-interest records returned by the API.
                                </div>
                              </div>
                            </div>
                          ) : (
                            <div className="max-h-[590px] overflow-y-auto p-3 sm:p-4">
                              <div className="space-y-2.5">
                                {(selectedCampaign.interestedUsers || []).map((user, index) => (
                                  <article
                                    ...
                                    <div className="flex items-start justify-between gap-3">
                                      <div className="min-w-0 flex-1">
                                        ...
                                        <div className="mt-2 flex flex-wrap gap-1.5">
                                          {user.projectType && (
                                            <Tag className="m-0 text-[10px]">{user.projectType}</Tag>
                                          )}
                                          {user.userRole && (
