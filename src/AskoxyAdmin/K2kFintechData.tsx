...
const K2kFintechData: React.FC = () => {
  const [records, setRecords] = useState<K2kFintechItem[]>([]);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `${BASE_URL}/ai-service/entity-records/k2k-fintech`,
        {
          params: { page, size },
        }
      );
  ...
  const columns: ColumnsType<K2kFintechItem> = [
    {
      ...
      >
        <div>
          ...
          <Text type="secondary" style={{ fontSize: 13 }}>
            View K2K Fintech contact records
          </Text>
        </div>
        <Button
          icon={<ReloadOutlined />}
