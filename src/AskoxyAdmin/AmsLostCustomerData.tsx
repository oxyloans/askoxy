...
const AmsLostCustomerData: React.FC = () => {
  const [records, setRecords] = useState<AmsLostCustomerItem[]>([]);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `${BASE_URL}/ai-service/entity-records/ams-lost-customer-data`,
        {
          params: { page, size },
        },
      );
  ...
  const columns: ColumnsType<AmsLostCustomerItem> = [
    {
    ...
    >
      ...
      >
        <div>
          ...
          >
            AMS Lost Customer Data
          </Title>
          <Text type="secondary" style={{ fontSize: 13 }}>
            View AMS lost customer contact records
          </Text>
        </div>
        <Button
          icon={<ReloadOutlined />}
...
export default AmsLostCustomerData;
