...
const TwoYearsServiceData: React.FC = () => {
  const [records, setRecords] = useState<TwoYearsServiceItem[]>([]);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `${BASE_URL}/ai-service/entity-records/two-years-service-data`,
        {
          params: { page, size },
        }
      );
  ...
  const columns: ColumnsType<TwoYearsServiceItem> = [
    {
    ...
    >
      ...
      >
        <div>
          ...
          >
            Two Years Service Data
          </Title>
          <Text type="secondary" style={{ fontSize: 13 }}>
            View Two Years Service contact records
          </Text>
        </div>
        <Button
          icon={<ReloadOutlined />}
...
export default TwoYearsServiceData;
