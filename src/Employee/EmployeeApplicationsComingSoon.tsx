...
const   EmployeeApplicationsComingSoon: React.FC = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<string>(STATUS_TABS[0].key);
  ...
  const loadApplications = useCallback(async (signal?: AbortSignal, isRefresh = false) => {
    ...
    try {
      const query = `userId=${encodeURIComponent(auth.id)}&page=${page}&size=${PAGE_SIZE}&status=${encodeURIComponent(statusFilter)}`;
      const response = await fetch(
        `${BASE_URL}/marketing-service/campgin/new-applicants-for-users-jobs-new?${query}`,
      ...
      if (!response.ok || result?.status === false) {
        throw new Error(
          result?.message || `Unable to load applications. Server returned ${response.status}.`
        );
      }
      ...
      setPageData({
        ...
        number: result?.currentPage ?? 0,
        first: (result?.currentPage ?? 0) === 0,
        last: (result?.currentPage ?? 0) + 1 >= (result?.totalPages ?? 1),
      });
  ...
  const updateCandidateStatus = async (applicant: Applicant, nextStatus: string) => {
    setUpdatingId(applicant.jobAppliedId);
    try {
      const response = await fetch(
        `${BASE_URL}/marketing-service/campgin/candidate/${applicant.jobAppliedId}/jobstatus?status=${encodeURIComponent(nextStatus)}`,
        { method: "PATCH", headers: { Accept: "application/json" } }
      );

      if (!response.ok) {
        throw new Error(`Unable to update status. Server returned ${response.status}.`);
      }

      toast.success(`Marked as ${nextStatus}.`);
      loadApplications(undefined, true);
    } catch (requestError) {
      const message =
        requestError instanceof Error ? requestError.message : "Unable to update candidate status.";
      toast.error(message);
    } finally {
      setUpdatingId(null);
    }
  ...
  return (
    <section className="eapps-page" aria-labelledby="employee-applications-title">
      ...
      <div className="eapps-summary" aria-label="Application summary">
        ...
        <div><small>{statusFilter} applications</small><strong>{pageData.totalElements ?? (loading ? "—" : 0)}</strong></div>
        <p>
          {pageData.totalElements === 1
            ? `1 candidate in ${statusFilter}.`
            : `${pageData.totalElements ?? 0} candidates in ${statusFilter}.`}
        </p>
      </div>

      <div className="eapps-tabs" role="tablist" aria-label="Application status">
        {STATUS_TABS.map((tab) => (
          <button
            key={tab.key}
            type="button"
            ...
            className={`eapps-tab ${statusFilter === tab.key ? "eapps-tab-active" : ""}`}
      ...
      {loading ? <LoadingState /> : error ? <ErrorState message={error} onRetry={() => loadApplications()} /> : visibleApplicants.length === 0 ? (
        <div className="eapps-state"><UsersRound /><h2>{applicants.length ? "No candidates match your search" : `No ${statusFilter.toLowerCase()} applications yet`}</h2><p>{applicants.length ? "Try a different candidate name, email or phone number." : "Applications to your posted jobs will appear here."}</p></div>
      ) : (
        <div className="eapps-list">
          {visibleApplicants.map((applicant) => (
            <article className="eapps-card" key={applicant.jobAppliedId}>
              <div className="eapps-avatar" aria-hidden="true">{initials(applicant.userName)}</div>
              <div className="eapps-card-main">
                <div className="eapps-card-top">
                  <div>
                    <h2>{applicant.userName || "Unnamed candidate"}</h2>
                    <p>Applied {formatDate(applicant.appliedAt)}</p>
                  </div>
                  <div className="eapps-top-right">
                    <span className="eapps-status">{applicant.applicationStatus || "Applied"}</span>
                    {statusFilter !== "Completed" && (
                      <button
                        type="button"
                        className="eapps-complete-btn"
                        onClick={() => updateCandidateStatus(applicant, "Completed")}
                        disabled={updatingId === applicant.jobAppliedId}
                      >
                        {updatingId === applicant.jobAppliedId ? (
                          <LoaderCircle className="eapps-spin" />
                        ) : (
                          <CheckCircle2 />
                        )}
                        <span>{updatingId === applicant.jobAppliedId ? "Updating..." : "Mark complete"}</span>
                      </button>
                    )}
                  </div>
                </div>
...
const applicationsStyles = `
  ...
  .eapps-tabs { display: flex; gap: 8px; margin-bottom: 14px; } .eapps-tab { padding: 8px 14px; border-radius: 999px; border: 1px solid rgba(255,255,255,.14); background: rgba(255,255,255,.05); color: rgba(226,231,250,.6); font: 750 .7rem Inter, sans-serif; cursor: pointer; } .eapps-tab-active { color: #fff; background: rgba(93,109,242,.28); border-color: rgba(144,156,255,.4); }
